...
"""
        ...
        if untracked_result.stdout.strip():
            ...
            for f in untracked_result.stdout.strip().split("\n"):
                lower = f.lower()
                if any(pat in lower for pat in [".env", "secret", "credential", ".key", ".pem", "token", ".crush", "node_modules/"]):
...
try:
    ...
    is_draft=settings.pr_draft_by_default,
            )

            draft_label = "[DRAFT]" if settings.pr_draft_by_default else ""
            logger.info(f"Created {draft_label} PR: {pr.get('pullRequestId')}")

            # Add comment instead of auto-closing
            if settings.add_pr_comment:
                await ado_client.add_pr_comment(
                    repo_url=repo_url,
                    pr_id=pr.get('pullRequestId'),
                    comment="""## AI Implementation Review Requested 👀

The AI agent has completed the implementation based on the work item requirements.
...
**If changes needed:** Request changes and the AI can iterate.
**If approved:** Merge the PR, then manually close the work item.

---
*This PR was created automatically by AI Coder. Manual closure required.*
""",
                )
                logger.info(f"Added comment to PR {pr.get('pullRequestId')}")

            # Keep work item in "New" state, just add history comment
            await ado_client.update_work_item(
                work_item_id=work_item_id,
                project_name=project_name,
                fields={
                    "System.History": f"AI Coder created draft PR #{pr.get('pullRequestId')} - awaiting review",
                },
            )
            logger.info(f"Work item {work_item_id} updated (state unchanged - manual closure required)")

        except Exception as e:
            logger.error(f"Failed to create PR or update work item: {e}")
            raise
...
async def start_queue_consumer():
    ...
    logger.info("Initializing queue clients...")
    ...
    logger.info("Starting queue consumer polling loop...")

    while True:
        try:
            messages = await loop.run_in_executor(
                None,
                lambda: list(queue_client.receive_messages(
                    max_messages=1,
            ...
            for message in messages:
                ...
                try:
                    ...
                    await loop.run_in_executor(
                        None, queue_client.delete_message, message
                    )
                    ...
                    if retry_count < settings.max_retries - 1:
                        ...
                        await loop.run_in_executor(
                            None, queue_client.send_message, json.dumps(task)
                        )
                        ...
                        await loop.run_in_executor(
                            None, dlq_client.send_message, message.content
                        )
                        logger.error(
